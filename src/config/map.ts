...
// 获取地图API密钥
export const getMapApiKey = (serviceType: MapServiceType = DEFAULT_MAP_SERVICE): string => {
  switch (serviceType) {
...
// 检查API密钥是否已配置
export const isApiKeyConfigured = (serviceType: MapServiceType = DEFAULT_MAP_SERVICE): boolean => {
  const key = getMapApiKey(serviceType);
  return key !== 'YOUR_TENCENT_MAP_KEY' && 
         key !== 'YOUR_AMAP_KEY' && 
