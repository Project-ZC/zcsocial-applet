<template>
  <view class="vertical-tabs">
    <view class="tabs-container">
      ...
      <view class="main-content" :class="{ 'main-content--diy': currentMainType === 'diy' }">
          ...
          <up-swipe-action>
            ...
            >
              <view class="good-card-title">
                <text>{{ i.name }}</text>
                <view class="man-action-btns" v-if="activeIndex === idx && props.isEdit">
                  <view class="edit-btn" @click="editCurrentCategory(i, idx)">
                    <up-icon name="edit-pen" size="36"></up-icon>
                  </view>
                  <view class="delete-btn" @click="deleteCurrentCategory(i, idx)">
              ...
              <drag-container
                ...
                class="good-list"
              >
                <template #default="{ moveItem, moveIndex, height, margin }">
                  <up-swipe-action-item
                    :index="moveItem.id || moveIndex"
                    @click="handleSwipeClick($event, moveItem, moveIndex)"
                    :options="options1"
                    :disabled="!props.isEdit"
                    v-model:show="moveItem.showSwipe"
                  >
                    <view class="good-wrap" @click="handleSwipeClick({ index: 0 }, moveItem, moveIndex)">
                      <good-card
                        :goodItem="moveItem"
                        :index="moveIndex"
                        :height="height"
                      ...
                      >
                        ...
                        <view class="good-status">
                          <up-tag
                            ...
                            plain
                            shape="circle"
                            :type="moveItem.stage === 'up' ? 'success' : 'error'"
                          >
                            {{ moveItem.stage === 'up' ? '已上架' : '已下架' }}
                          </up-tag>
                        </view>
                        <template #title>
                          <view class="free-tag">
                            <up-tag v-if="!props.isEdit" size="mini" shape="circle" :type="'success'">门票免费</up-tag>
                          </view>
                        </template>
                        <template #actions="{ item, index }">
                          <!-- <slot
														...
														:index="index"
													></slot> -->
                          <!-- <view class="ope-btn" v-if="props.isEdit">
														<up-button
															type="primary"
															size="mini"
															shape="circle"
                          ...
                          <view class="cart-btn" v-if="props.showActionBtn">
                            <up-button type="burgundy" size="small" shape="circle" @click="openAddToCart(item)">
                              加入购物车
                            </up-button>
                            <view class="cart-badge">
                              <up-badge :max="99" type="error" :value="item.cartNum || 0"></up-badge>
                            </view>
                          </view>
                        </template>
                      </good-card>
              ...
              <emptyData v-else :text="props.isEdit ? '当前细分类还没有商品' : '暂无商品'">
                <template #subtext v-if="props.isEdit">
                  <text @click="openAddDrinkModal(i, idx)">请点击添加商品</text>
                </template>
              </emptyData>
            </view>
          </up-swipe-action>
        ...
        <emptyData v-else :text="props.isEdit ? '当前暂无细分类' : '暂无商品'" subtext="">
          <template #subtext v-if="props.isEdit">
...
import { ref, watch, nextTick, onMounted, getCurrentInstance, toRefs, reactive } from 'vue';
...
interface Emits {
  ...
  (e: 'swipeClick', value: { item: any; index: number }): void;
  (e: 'openAddToCart', value: any): void;
  (e: 'editProduct', value: { item: any; index: number }): void;
}
...
const editProduct = (item: any, index: number) => {
  emit('editProduct', item, index);
};
...
const editCurrentCategory = (item, index) => {
...
const deleteCurrentCategory = (item, index) => {
...
const openAddDrinkModal = (item, index) => {
...
const handleSwipeClick = (e: any, item: any, index: number) => {
  emit('swipeClick', e, item, index);
};
