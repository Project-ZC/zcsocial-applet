<template>
  <view class="navigation" :style="{ background: bgcolor }">
    ...
    <view class="nav-bar-inner">
      <view class="address-box" :style="{ height: menuButton.height + 'px' }" @tap="handleLocationClick">
        <up-icon name="map" color="var(--text-1)" />
        <text class="address">{{ currentAddress }}</text>
        <!-- <up-icon v-if="isLocating" name="loading" color="var(--text-1)" /> -->
      </view>
      <view
        class="search-box"
        :style="{
          width: screenInfo.screenWidth - menuButton.width - 80 + 'px',
        }"
      >
        <up-search
...
import { getAllShopConfig } from '@/api/shopManage';
...
// 逆地理编码
const reverseGeocoder = async (longitude: number, latitude: number) => {
  try {
    ...
    if (res.data) {
      ...
      const address = result.formatted_addresses || '未知地址';
      currentAddress.value = result.addressComponent.city;
    }
  } catch (error) {}
  // // 检查是否有配置的API密钥
  ...
  //         const address = result.formatted_addresses?.recommend || result.address || '未知地址';
...
const setMockAddress = (longitude: number, latitude: number) => {
  ...
  setTimeout(() => {
    ...
    } else if (longitude > 121.4 && longitude < 121.5 && latitude > 31.2 && latitude < 31.3) {
      currentAddress.value = '上海市';
    } else if (longitude > 113.2 && longitude < 113.3 && latitude > 23.1 && latitude < 23.2) {
      currentAddress.value = '广州市';
    } else if (longitude > 114.0 && longitude < 114.1 && latitude > 22.5 && latitude < 22.6) {
...
const handleSearch = async (keyword: string) => {
  ...
  try {
    const res = await getAllShopConfig({
      name: keyword.trim(),
      // pageNum: 1,
      // pageSize: 20,
    });
