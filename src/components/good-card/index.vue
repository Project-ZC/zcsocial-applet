<template>
  <!-- 商品明细card -->
  <view class="good-item z-glass-card" :style="{ height: props.height + 'px' }" v-if="props.goodItem">
    <!-- <movable-area> -->
    <!-- 第一列：商品图片 -->
    ...
    <view class="good-info">
      ...
      <view class="z-tips">{{ availableSkus?.[0]?.size }}{{ availableSkus?.[0]?.unit }}</view>
      <!-- 多规格商品显示 -->
...
const hasMultipleSkus = computed(() => {
  return props.goodItem?.productSkus?.length > 1;
});
