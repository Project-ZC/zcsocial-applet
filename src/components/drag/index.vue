<template>
  <view
    ...
    :style="{
      height: state.controlsSize.height * state.controlsPositionArray.length + state.margin.margin_y * 2 + 'px',
    }"
  >
    <view
...
const initControlsPosition = () => {
  ...
  for (let i = 0, j = 0; i < props.list.length; i++, j++) {
    tempArray[i] = {
      ...
      top: j * (state.controlsSize.height + state.margin.margin_y) + state.margin.margin_y,
    };
  }
