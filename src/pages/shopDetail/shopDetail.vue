    ...
    <view class="shop-detail">
      ...
      <view class="shop-form z-glass-card">
        <up-form :model="form" ref="formRef" :rules="state.rules" labelPosition="top" labelWidth="auto">
          ...
          <up-form-item label="营业时间" prop="openTime">
            <view class="time-picker-row">
              <view @click="showOpenTimePicker = true">
                <up-input class="time-picker" v-model="form.openTime" readonly placeholder="请选择" />
              </view>
              <text class="time-separator">至</text>
              <view @click="showCloseTimePicker = true">
                <up-input class="time-picker" readonly v-model="form.closeTime" placeholder="请选择" />
              </view>
            </view>
          </up-form-item>
          ...
          <!-- 店铺标签 -->
          <up-form-item label="店铺标签" prop="tags" v-if="false">
            <view class="tags-container">
              <!-- 已选标签展示 -->
              <view class="selected-tags">
                <view class="tags-list" v-if="form.tags.length > 0">
                  <view class="tag-item selected" v-for="(item, index) in form.tags" :key="index">
              ...
              <view class="tag-input-section">
                ...
                <!-- 可选标签列表 -->
                <view class="available-tags" v-if="showAvailableTags && availableTags.length > 0">
                  <view class="tags-title">可选标签：</view>
                  <view class="tags-container">
                    <view class="tag-item available" v-for="tag in availableTags" :key="tag.id" @click="selectTag(tag)">
                ...
                <view class="search-suggestions" v-if="showSuggestions && filteredTags.length > 0">
                  ...
                  <view class="suggestions-list">
                    <view class="suggestion-item" v-for="tag in filteredTags" :key="tag.id" @click="selectTag(tag)">
          ...
          <up-form-item label="店铺标签22" prop="tags">
            <TagSelector
              style="width: 100%"
              v-model:selectedTags="state.tags"
              :tags="tagList"
              :maxTags="5"
              :allowCustom="false"
            />
...
const availableTags = computed(() => {
  ...
  return tagList.value.filter(tag => !selectedNames.includes(tag.name.toLowerCase()));
});
...
const addTag = () => {
  ...
  if (tagName && form.tags.length < 5) {
    // 检查是否已存在相同名称的标签
    const existingTag = form.tags.find(tag => tag.toLowerCase() === tagName.toLowerCase());
    if (existingTag) {
      uni.showToast({
        title: '标签已存在',
        icon: 'none',
...
const selectTag = tag => {
  ...
  // 检查是否已存在相同名称的标签
  const existingTag = form.tags.find(t => t.toLowerCase() === tag.name.toLowerCase());
  if (existingTag) {
    uni.showToast({
      title: '标签已存在',
      icon: 'none',
