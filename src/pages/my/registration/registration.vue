			...
			<view class="application-list">
				...
				>
					...
					<view class="application-content">
						...
						<view
							class="application-row photo-gallery-row"
							v-if="item.uploadedPhotos && item.uploadedPhotos.length"
						>
				...
				<emptyData
					height="60vh"
					:text="state.emptyDescMap[state.selectedStatus]"
					v-if="!state.filteredList.length"
				>
					<template #subtext>
						{{ state.emptyDescMap[state.selectedStatus] }}</template
					>
				</emptyData>
			</view>
		</view>
...
const filterList = () => {
	...
	if (state.selectedStatus !== "all")
		list = list.filter((x: any) => x.status === state.selectedStatus);
	if (state.searchKeyword && state.searchKeyword.trim()) {
		const kw = state.searchKeyword.trim().toLowerCase();
		list = list.filter(
			(x: any) =>
				(x.shopName || "").toLowerCase().includes(kw) ||
				(x.activityName || "").toLowerCase().includes(kw)
		);
	}
	...
	state.tabList = state.tabList.map((t: any) => ({
		...
		count:
			t.status === "all"
				? state.applyList.length
				: state.applyList.filter((x: any) => x.status === t.status).length,
	}));
};
...
const deleteApplication = (item: any) => {
	...
	if (item.status === "pending")
		confirmContent = `确定要删除"${item.activityName}"的报名吗？删除后将无法恢复。`;
...
.application-item {
	...
	margin-bottom: 20rpx;
	padding: 24rpx;
	border: 1rpx solid var(--border-2);
}
.application-header {
	display: flex;
