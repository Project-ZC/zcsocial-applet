	...
	<pageWrapper showFooter :scrollTop="60">
				...
				<view class="avatar-section z-glass-card">
					<view class="avatar-container">
						...
						>
							<template #default>
								<up-avatar
									:src="getDownloadUrl(state.userInfo.avatar)"
									size="140rpx"
								></up-avatar>
							</template>
							<template #tips>
								<text class="edit-text" @click="triggerAvatarUpload"
									>点击更换头像</text
								>
							</template>
						</UploadFile>
							...
							<view class="form-value">
								{{
									state.genderOptions.find(
										(item) => item.code === state.userInfo.gender
									)?.name || "请选择"
								}}
							</view>
							<view class="form-icon">
              ...
              <view class="form-icon">
                <view class="icon-arrow-right"></view>
						...
						<view class="form-item">
							...
							<view class="form-value">{{ state.userInfo.mobile }}</view>
						</view>
					</view>
				</view>
				...
				<view class="form-card z-glass-card">
					...
					<view class="bio-container">
						...
						<text class="word-count"
							>{{ state.userInfo.introduce.length }}/200</text
						>
					</view>
				</view>
			</view>
		...
		<template #footer>
			<view class="save-button">
				<up-button class="" type="primary" shape="circle" @click="saveProfile"
					>保存</up-button
				>
			</view>
		</template>
				...
				<view class="picker-body">
					...
					>
						<text
							...
							:class="{ active: state.userInfo.gender === item.code }"
						>
							{{ item.name }}
						</text>
						<view
							class="icon-check"
							v-if="state.userInfo.gender === item.code"
						></view>
					</view>
				</view>
			</view>
...
const confirmDatePicker = () => {
	...
	state.userInfo.birthday = `${date.getFullYear()}-${
		date.getMonth() + 1
	}-${date.getDate()}`;
	closeDatePicker();
};
