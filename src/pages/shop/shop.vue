...
<template>
	<pageWrapper :showTabbar="true">
		...
		<ShopSwitcher
			ref="shopSwitcherRef"
			v-if="
				state.shopList.length > 0 && userStore.checkPermission('shop/switch')
			"
		...
		>
			...
			<view class="shop-info-card z-glass-card">
				<view class="shop-header">
					...
					<view class="shop-basic-info">
						...
						<view class="shop-date"
							>营业时间：{{ shopInfo.businessHours }}</view
						>
					</view>
				...
				<view
					...
					@click="openShopModal"
					v-if="userStore.checkPermission('shop/switch')"
				>
...
const filteredCellList = computed(() => {
	return (
		state.cellList
			...
			.filter(
				(main: any) =>
					!main.permission || userStore.checkPermission(main.permission)
			)
			.map((main) => ({
				...main,
				children: main.children.filter(
					(item) =>
						!item.permission || userStore.checkPermission(item.permission)
				),
			}))
...
const openShopModal = () => {
	...
	const currentShopData = state.shopList.find(
		(shop) => shop.shopConfig?.shopId === shopInfo.value?.shopId
	);
...
const GetShopConfigList = async () => {
	try {
		...
		if (res.data?.length) {
			...
			shopInfo.value.businessHours = getCurrentBusinessHours(
				shopInfo.value.shippingTimeList
			)?.time;
