...
<template>
  <pageWrapper :showTabbar="true">
    ...
    >
      ...
      <view class="order-content" v-show="state.currentTab === 'accept-order'">
        <Order />
      </view>
    </view>

    <!-- 店铺二维码弹窗 -->
    <up-popup :show="state.showQRCodeModal" mode="center" :closeOnClickOverlay="true" @close="closeQRCodeModal">
      <view class="z-modal">
        <view class="modal-header">
          <text class="modal-title">店铺二维码</text>
          <up-icon name="close" color="#999" size="20" @click="closeQRCodeModal"></up-icon>
        </view>
        <scroll-view scroll-y>
          <view class="modal-body">
            <view class="qrcode-info">
              <view class="user-info">
                <up-avatar :src="getDownloadUrl(shopInfo.logo)" size="100rpx"></up-avatar>
                <view class="user-text">
                  <text class="user-name">{{ shopInfo.name }}</text>
                </view>
              </view>
              <view class="qrcode-container">
                <up-image class="qrcode-image" :src="shopInfo.qrCodeUrl" mode="aspectFit"></up-image>
                <!-- <text class="qrcode-tip">长按保存到相册</text> -->
              </view>
            </view>
          </view>
        </scroll-view>
        <view class="modal-footer">
          <up-button type="cancel" shape="circle" @click="closeQRCodeModal">关闭</up-button>
          <up-button type="primary" shape="circle" @click="saveQRCodeToAlbum">保存到相册</up-button>
        </view>
      </view>
    </up-popup>
  </pageWrapper>
...
import { previewImage, getCurrentBusinessHours, saveImgToAlbum } from '@/utils/util';
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          title: '桌位管理',
          icon: 'shop-table',
          url: PAGE_LIST.SHOP_TABLE_MANAGE,
          showArrow: true,
        ...
        },
        {
          title: '门票设置',
          icon: 'shop-ticket',
          url: PAGE_LIST.SHOP_TICKET_SETTINGS,
          showArrow: true,
...
const saveQRCodeToAlbum = () => {
  saveImgToAlbum(shopInfo.value.qrCodeUrl);
};
