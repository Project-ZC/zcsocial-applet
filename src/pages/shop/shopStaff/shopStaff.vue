<template>
  <pageWrapper class="staff">
    <up-sticky>
      <!-- 顶部导航栏 -->
      <!-- <up-navbar class="z-navbar" :title="'员工管理'" :border="true" leftIcon="">
        <template #right>
      ...
      <view class="z-page-header z-glass-card">
        ...
        <up-button type="gradient1" @click="openAddModal">添加员工</up-button>
      </view>
      <!-- 角色筛选 -->
    ...
    <view class="staff-list">
      ...
      >
            ...
            <view class="staff-item">
              ...
              <view class="staff-actions">
                <view class="action-edit" @click="openEditModal(item)">
                  <up-icon name="edit-pen" size="26"></up-icon>
                </view>
                <view class="action-delete" @click="openDeleteModal(item)">
                  <up-icon name="trash" color="var(--danger-6)" size="26"></up-icon>
                </view>
              </view>
            </view>
          </view>
        ...
        <!-- 员工列表为空状态 -->
        <emptyData text="暂无员工" height="80%" v-else-if="!state.isLoading && state.filteredStaffList.length === 0">
          <template #subtext>
  ...
  <up-popup :show="state.showModal" @close="closeModal" round="20rpx">
    <view class="z-modal">
        ...
        <view class="modal-body">
          ...
          <view v-if="state.resultList.length > 0">
            ...
            <view class="form-item">
              ...
              <up-input v-model="state.tempStaff.nickname" placeholder="请输入员工呢称" disabled />
            </view>
            <!-- <view class="form-item">
      ...
      <view class="modal-footer" v-if="state.resultList.length > 0">
        <up-button type="cancel" @click="closeModal" shape="circle">取消</up-button>
