	...
	<pageWrapper class="staff">
			...
			<scroll-view scroll-x class="role-filter-scroll">
				<view class="role-filter">
					<view
						...
						:class="{ active: state.currentRole === 'all' }"
					>
						全部
					</view>
					<view
		...
		<view class="staff-list">
			...
			>
					...
					<view v-for="(item, index) in state.filteredStaffList" :key="item.id">
						<view class="staff-item">
							<view class="staff-avatar">
								<up-avatar
									:src="getDownloadUrl(item.userAvatar)"
									size="50"
								></up-avatar>
							</view>
							<view class="staff-info">
								<view class="staff-header">
									...
									<view
										class="staff-roleIds"
										v-if="item.userRoleNames && item.userRoleNames.length > 0"
									>
										<text
											class="staff-role"
											v-for="(permission, pIndex) in item.userRoleNames"
											:key="pIndex"
										>
											{{ permission }}
										</text>
								...
								<view
									class="staff-contact"
									v-if="item.userMobile || item.userId"
								>
									<text v-if="item.userMobile"
										>电话: {{ item.userMobile }}</text
									>
									<text class="line" v-if="item.userMobile && item.userId"
										>|</text
									>
				...
				<view
					class="empty-state"
					v-else-if="!state.isLoading && state.filteredStaffList.length === 0"
				>
					<EmptyData text="暂无员工">
						<template #subtext>
	...
	<up-popup :show="state.showModal" @close="closeModal" round="20rpx">
		<view class="z-modal">
				...
				<view class="modal-body">
					...
					<view v-if="state.resultList.length > 0">
						<view class="form-item">
							...
							<view
								class="avatar-picker"
								@click="state.isEdit ? null : chooseImage()"
							>
								<up-avatar
									:src="
										getDownloadUrl(state.tempStaff.avatar) ||
			...
			<view class="modal-footer" v-if="state.resultList.length > 0">
				...
				<up-button type="primary" shape="circle" @click="saveStaff"
					>确定</up-button
				>
			</view>
		</view>
	...
	>
		<view class="qrcode-container">
			...
			<view class="qrcode-actions">
				<up-button class="save-btn" @click="saveQRCodeToAlbum"
					>保存到相册</up-button
				>
...
const selectPermission = (permissionId: string) => {
	...
	if (state.tempStaff.roleIds.includes(permissionId)) {
		...
		// 根据选中的权限生成职位描述
		const selectedRole = state.roleList.find(
