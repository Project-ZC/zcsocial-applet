<template>
  <view class="z-modal-lg z-modal">
    <view class="modal-header">
      <text class="modal-title">{{ props.type === 'edit' ? '编辑酒料' : '添加酒料' }}</text>
      ...
      >
        ...
        >
          ...
          <up-form-item label="描述" prop="description">
            <up-textarea v-model="state.form.description" placeholder="请输入酒料描述" />
          </up-form-item>
          ...
          <up-form-item label="计量单位" prop="unit">
            <up-input v-model="state.form.unit" placeholder="例如：份、杯、瓶等" />
          </up-form-item>
...
const saveDiyIngredient = async () => {
  try {
    ...
    const diyTypeIndex = state.diyTypes.findIndex(type => type.id === state.form.diyTypeId);
    if (diyTypeIndex !== -1) {
      if (props.type === 'edit') {
        const ingredientIndex = state.diyTypes[diyTypeIndex].ingredients.findIndex(i => i.id === state.form.id);
