				...
				>
					...
					<up-form-item label="添料组管理" prop="productIngredients">
						<view class="ingredient-groups-container">
							...
							<view
								v-if="
									state.form.productIngredients?.length > 0 &&
									state.showIngredientGroups
								"
							>
								<drag-container
			...
			for (const key in props.productFormData.productOptions) {
				...
				options.push({
					...
					options: formatValuesToOptions(
						props.productFormData.productOptions[key],
						true
					),
				});
			}
