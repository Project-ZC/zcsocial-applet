        ...
        <up-form :model="state.form" ref="formRef" :rules="state.rules" labelPosition="top" labelWidth="auto">
          ...
          <up-form-item label="模板选项">
            <view class="options-editor">
              <view class="option-row" v-for="(opt, idx) in state.form.options" :key="idx">
                <text class="option-label">名称</text>
                <up-input v-model="state.form.options[idx]" placeholder="请输入选项名称">
                  <template #suffix>
...
const state = reactive({
  ...
  templateActions: [
    { name: '冰', type: '冰', value: 'ice' },
    { name: '度数', type: '度数', value: 'degree' },
    { name: '甜度', type: '甜度', value: 'sweet' },
    { name: '加工方式', type: '加工方式', value: 'process' },
  ],
  templates: {
    ice: ['不加冰', '少冰', '正常冰'],
...
const onSelectTemplate = (action: any) => {
  ...
  state.form.options = list.slice();
  if (!state.form.name) state.form.name = action.name;
  state.showTemplateSheet = false;
  state.form.type = action.type;
};

const addOption = () => state.form.options.push('');
const removeOption = (idx: number) => state.form.options.splice(idx, 1);
...
onMounted(() => {
  ...
  state.templateActions.forEach(item => {
    item.name += '(' + state.templates[item.value].join('、') + ')';
  });
});
</script>
