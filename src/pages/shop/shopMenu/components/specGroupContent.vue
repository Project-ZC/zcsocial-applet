					...
					<up-form-item label="模板选项">
						<view class="options-editor">
							...
							>
								...
								<up-input
									v-model="state.form.values[idx].value"
									placeholder="请输入选项名称"
								>
									<template #suffix>
										<up-icon
...
const removeOption = (idx: number) =>
	removeOptionFromArray(state.form.values, idx);
...
onMounted(() => {
	...
	if (props.specGroupFormData) {
		...
		if (props.type === "edit") {
			...
			const a = state.templateActions.find(
				(item) => item.value === props.specGroupFormData.key
			);
