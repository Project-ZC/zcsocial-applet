  ...
  <pageWrapper>
    <view class="container">
      ...
      <view class="menu-tabs">
        <view class="menu-tab" :class="{ active: state.menuType === 'regular' }" @click="switchMenuType('regular')">
          常规酒单
        </view>
        <view class="menu-tab" :class="{ active: state.menuType === 'diy' }" @click="switchMenuType('diy')">
          DIY酒料
        </view>
      </view>
        ...
        <view class="drink-list" v-if="filteredDrinks.length > 0">
          <view class="drink-item" v-for="(item, index) in filteredDrinks" :key="item.id">
            <view class="drink-info">
              ...
              <view class="drink-detail">
                ...
                <view class="drink-price-row">
                  ...
                  <text class="drink-original-price" v-if="item.originalPrice && item.originalPrice > item.price">
                    ¥{{ item.originalPrice }}
                  </text>
                </view>
              </view>
            ...
            <view class="drink-actions">
              <up-button type="warning" class="edit-btn" plain shape="circle" @click="openEditDrinkModal(item.id)">
                编辑
              </up-button>
              <up-button type="error" class="delete-btn" plain shape="circle" @click="openDeleteDrinkModal(item.id)">
                删除
              </up-button>
            </view>
          </view>
...
const filteredDiyIngredients = computed(() => {
  ...
  return state.diyIngredients.filter(item => item.typeId === state.currentDiyTypeId).sort((a, b) => a.sort - b.sort);
});
