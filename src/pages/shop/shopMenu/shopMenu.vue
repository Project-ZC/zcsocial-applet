      ...
      >
        ...
        <template #content>
          <view class="top-content">
            <view class="z-glass-card diy-price-card" v-if="state.currentMainType === 'diy'">
              <view class="card-header">
                <text class="card-title">DIY饮品价格</text>
            ...
            <view class="tab0-buttons z-glass-card">
              <up-button type="success" shape="circle" @click="openAddCategoryModal">添加细分类</up-button>
              <up-button type="primary" shape="circle" @click="openAddDrinkModal">添加商品</up-button>
            </view>
          </view>
        </template>
...
import { getProductCatalogAll, getProductList, deleteProductCatalog, deleteProduct } from '@/api/product';
...
const editGoods = (item, index: number) => {
  state.productFormData = cloneDeep(item);
...
const GetProductList = async (catalogId: string) => {
  ...
  const index = state.tabs.findIndex(tab => tab.id == catalogId);
  try {
    const params = {
      shopId: state.shopId,
      catalogId,
    ...
    if (res.data && Array.isArray(res.data)) {
      ...
      // 找到对应的分类并更新 children
      if (targetTab) {
        state.tabs[index].children = products;
        state.tabs[index].hasLoaded = true; // 标记为已加载
...
// 打开添加编辑酒水弹框
const openAddDrinkModal = data => {
  // state.tabs[0].children = [
  // 	{ id: 1, name: "商品1" },
  // 	{ id: 2, name: "商品2" },
  // 	{ id: 3, name: "商品3" },
  // ];
