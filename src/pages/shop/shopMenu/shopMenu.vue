<template>
  <pageWrapper>
        ...
        <template #content>
          <view class="content">
            <view v-if="state.activeTab != 1" class="tab0-buttons">
              <up-button type="success" @click="openAddCategoryModal">添加细分类</up-button>
              <up-button type="primary" @click="openAddDrinkModal">添加商品</up-button>
            </view>
            <view v-if="state.activeTab >= 2">
              <view class="category-management-section">
                <view class="section-header">
                  <text class="section-title">{{ state.tabs[state.activeTab]?.title }}管理</text>
            ...
            <goods :goodsList="filteredOrders" v-if="filteredOrders.length > 0">
              <template #actions="{ item, index }">
                <up-button type="primary" size="mini" @click="editGoods(item, index)" class="action-btn">
                  编辑
                </up-button>
                <up-button type="error" size="mini" @click="deleteGoods(item, index)" class="action-btn">
                  删除
                </up-button>
                <up-button type="success" size="mini" @click="upGoods(item, index)" class="action-btn">上移</up-button>
                <up-button type="warning" size="mini" @click="downGoods(item, index)" class="action-btn">
                  下移
                </up-button>
              </template>
            </goods>
    ...
    <up-popup :show="state.showCategoryModal" mode="center" @close="closeCategoryModal">
      <view class="z-modal-lg z-modal">
        <view class="modal-header">
          <text>{{ state.isEditingCategory ? '编辑细分类' : '添加细分类' }}</text>
        ...
        <view class="modal-body">
          ...
          >
            <up-form-item label="细分类名称" prop="name">
              <up-input v-model="state.tempCategory.name" placeholder="请输入细分类名称" />
            </up-form-item>
            <up-form-item label="排序 (数字越小排序越靠前)" prop="sort">
              <up-input v-model="state.tempCategory.sort" placeholder="请输入排序数字" type="number" />
            </up-form-item>
          </up-form>
        </view>
        ...
        <scroll-view scroll-y="true">
          <view class="modal-body">
            <view class="image-upload">
              <UploadFile v-model="state.tempDrink.image" :max-count="1"></UploadFile>
            </view>
            ...
            >
              ...
              <up-form-item label="所属细分类" prop="categoryId">
                <view class="category-selector" @click="showCategorySelector">
                  <up-input v-model="state.tempDrink.categoryName" readonly placeholder="请选择细分类">
                    <template #suffix>
              ...
              <up-form-item label="描述" prop="description">
                <up-textarea v-model="state.tempDrink.description" placeholder="请输入酒水描述" />
              </up-form-item>
              ...
              <up-form-item label="容量组管理" prop="capacityGroups">
                <view class="capacity-groups-container">
                  ...
                  <view
                    ...
                    v-if="state.tempDrink.capacityGroups?.length > 0 && state.showCapacityGroups"
                  >
                    <drag-container :list="state.tempDrink.capacityGroups" :controlsSize="{ height: 50 }">
              ...
              <up-form-item label="添料组管理" prop="ingredientGroups">
                <view class="ingredient-groups-container">
                  ...
                  <view v-if="state.tempDrink.ingredientGroups?.length > 0">
                    <up-dragsort :initialList="state.tempDrink.ingredientGroups" direction="vertical">
                      <template #default="{ item, index }">
              ...
              <view class="form-tip">
                <text class="tip-text">设置顾客单次点单此酒水的最大数量，0表示不限制</text>
              </view>
            </up-form>
          </view>
        </scroll-view>
        ...
        <view class="modal-body">
          ...
          >
            ...
            >
              ...
              <up-form-item label="描述" prop="description">
                <up-textarea v-model="state.tempDiyIngredient.description" placeholder="请输入酒料描述" />
              </up-form-item>
    ...
    <up-popup :show="state.showCapacityGroupModal" mode="center" @close="closeCapacityGroupModal">
      <view class="z-modal-lg z-modal">
        <view class="modal-header">
          <text>{{ state.isEditingCapacityGroup ? '编辑容量组' : '添加容量组' }}</text>
        ...
        <view class="modal-body">
          ...
          >
            ...
            <up-form-item label="容量价格 (元)" prop="price">
              <up-input v-model.number="state.tempCapacityGroup.price" placeholder="请输入价格" type="number" />
            </up-form-item>
          </up-form>
        </view>
        <view class="modal-footer">
    ...
    <up-popup :show="state.showIngredientGroupModal" mode="center" @close="closeIngredientGroupModal">
      <view class="z-modal-lg z-modal">
        <view class="modal-header">
          <text>{{ state.isEditingIngredientGroup ? '编辑酒料' : '添加酒料' }}</text>
        ...
        <view class="modal-body">
          ...
          >
            <up-form-item label="酒料名称" prop="name">
              <up-input v-model="state.tempIngredientGroup.name" placeholder="如：珍珠/椰果/桂花" />
            </up-form-item>
            <up-form-item label="酒料金额 (每项)" prop="price">
              <up-input v-model.number="state.tempIngredientGroup.price" placeholder="请输入酒料金额" type="number" />
            </up-form-item>
        ...
        <view class="modal-footer">
          ...
          <up-button type="primary" @click="saveIngredientGroup">确定</up-button>
        </view>
      </view>
    </up-popup>
...
const filteredOrders = computed(() => {
  ...
  return state.orders.filter(order => order.type === state.tabs[state.activeTab].type);
});
// 筛选tags数据中下表大于2的分类数据
    ...
    if (diyTypeIndex !== -1) {
      if (state.isEditingDiyIngredient) {
        const ingredientIndex = state.diyTypes[diyTypeIndex].ingredients.findIndex(
          i => i.id === state.tempDiyIngredient.id
        );
...
const getSelectedCategoryName = () => {
  const selectedCategory = categories.value.find(cat => cat.type === state.tempDrink.categoryId);
...
const saveCapacityGroup = async () => {
  try {
    ...
    if (state.isEditingCapacityGroup) {
      // 编辑模式
      const index = state.tempDrink.capacityGroups.findIndex(g => g.id === state.tempCapacityGroup.id);
      if (index !== -1) {
...
const saveIngredientGroup = async () => {
  try {
    ...
    const drinkIndex = state.tempDrink.ingredientGroups.findIndex(g => g.id === state.tempIngredientGroup.id);
    if (drinkIndex !== -1) {
