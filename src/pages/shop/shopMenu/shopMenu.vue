        ...
        <template #content>
          <view class="content">
            <view class="z-glass-card" v-show="state.currentMainType === 'diy'">
              <view>DIY酒水价格</view>
              <view>
                <up-input v-model="state.diyPrice" placeholder="请输入DIY酒水价格" type="number"></up-input>
              </view>
            </view>
            ...
            <view v-for="(i, idx) in state.tabs" :key="i.type" class="good-card content-item" :id="`sec-${idx}`">
              <view class="good-card-title">
                <text>{{ i.title }}</text>
                <view class="man-action-btns" v-if="Number(state.activeTab) === idx">
              ...
              <view v-if="i.children?.length > 0">
                <template v-for="(child, index) in i.children" :key="child.id">
                  <good :goodItem="child" :index="index">
                    <template #actions="{ item, index }">
                      ...
                      <up-button type="success" size="mini" @click="upGoods(item, index)" class="action-btn">
                        上移
                      </up-button>
...
const openAddDrinkModal = () => {
  // 仅细分类下可添加商品
  if (state.tabs.length === 0 || state.activeTab === undefined || state.activeTab === null) {
