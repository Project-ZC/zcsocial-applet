<template>
  <pageWrapper showFooter ref="pageWrapperRef" :scrollTop="100">
    ...
    <template #footer>
      <view class="save-button">
        <up-button class="" type="gradient1" shape="circle" @click="saveShopInfo">保存</up-button>
      </view>
    </template>
  </pageWrapper>
  <!-- 营业时间选择器 -->
...
const tagFocus = () => {
  // console.log(1234, pageWrapperRef.value);
  // pageWrapperRef.value.setScrollIntoViewId({ id: 'page-bottom-anchor', scrollTop: 100 });
};
const tagBlur = () => {
  // console.log(1234, pageWrapperRef.value);
  // pageWrapperRef.value.setScrollIntoViewId({ id: '', scrollTop: 100 });
};
...
const GtShopDetail = () => {
  getShopDetail({
    ...
    if (form.shippingTimeList?.length) {
      const currentBusinessHours = getCurrentBusinessHours(form.shippingTimeList);
      if (currentBusinessHours) {
        form.closeTime = currentBusinessHours.closeTime;
        form.openTime = currentBusinessHours.openTime;
      }
    }
  });
};
