<template>
  <view class="ordering">
    <tabs :data="classifyList" />
    <content :data="classifyList" :showClassifyIndex="showClassifyIndex" />
  </view>
</template>

<script setup lang="ts">
import content from './omponents/content.vue';
...
// refs
const uniShoppingTrolleyRef = ref();
...
const tapGoodsDetails = (e: GoodsItem) => {
  uni.navigateTo({
    url: `/pages/goods-details/goods-details?data=${encodeURIComponent(JSON.stringify(e))}`,
  });
};
  ...
  switch (type) {
    case 1:
      ...
      if (goodsNum.value === 0) {
        show.value = false;
        setTimeout(() => {
          if (uniShoppingTrolleyRef.value) {
            uniShoppingTrolleyRef.value.isCartShow(); // 回调组件isCartShow方法
          }
        }, 20);
      }
      break;
